...
func (g *EksGenerator) InitResources() error {
	...
	for p.HasMorePages() {
		...
		for _, clusterName := range page.Clusters {
			g.getNodeGroups(clusterName, svc)
			g.Resources = append(g.Resources, terraformutils.NewSimpleResource(
				clusterName,
				clusterName,
				"aws_eks_cluster",
	...
	for _, resource := range g.Resources {
		if resource.InstanceInfo.Type == "aws_eks_node_group" {
			if _, ok := resource.Item["launch_template"]; ok {
				if _, ok := resource.Item["launch_template"].([]interface{})[0].(map[string]interface{})["id"]; ok {
					delete(resource.Item["launch_template"].([]interface{})[0].(map[string]interface{}), "id")
				}
			}
			if _, ok := resource.Item["update_config"]; ok {
				if _, ok := resource.Item["update_config"].([]interface{})[0].(map[string]interface{})["max_unavailable_percentage"]; ok {
