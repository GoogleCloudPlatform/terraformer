...
func (g *UserGenerator) createUserResources(users []sumologic.UserModel) []terraformutils.Resource {
	resources := make([]terraformutils.Resource, len(users))
	for i, user := range users {
		resource := terraformutils.NewSimpleResource(
			user.Id,
			fmt.Sprintf("%s-%s-%s", user.FirstName, user.LastName, user.Id),
			"sumologic_user",
			g.ProviderName,
			[]string{})
...
func (g *UserGenerator) InitResources() error {
	...
	resources := g.createUserResources(users)
